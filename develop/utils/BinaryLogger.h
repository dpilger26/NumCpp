...
namespace chimera::algs::common::utils::logger
{
	namespace detail
	{
		...
		public:
			 ...
			 * @param outputDir: the directory to place the output log
			 */
			explicit BinaryDataLogger(std::string_view outputDir) : BinaryDataLogger(std::filesystem::path(outputDir))
			{
			}
			...
			void log(const_pointer dataElements, std::size_t numElements)
			{
				...
				std::for_each(dataElements, dataElements + numElements, [this](const_reference dataElement) { log(dataElement); });
			}
